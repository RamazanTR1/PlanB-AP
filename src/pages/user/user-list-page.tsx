...
const sortOptions = [
  ...
  { label: "Güncellenme Tarihi (Yeni)", value: "updatedAt,desc" },
  { label: "Güncellenme Tarihi (Eski)", value: "updatedAt,asc" },
  { label: "Kullanıcı Adı (A-Z)", value: "username,asc" },
  { label: "Kullanıcı Adı (Z-A)", value: "username,desc" },
...
export default function UserListPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useUserList(debouncedSearch, page, size, sort[0] || "createdAt,desc");
  const deleteUserMutation = useDeleteUserById();
  const { openDeleteModal, DeleteModal } = useDeleteConfirmation();

  const users = userListResponse?.content || [];
  const totalPages = userListResponse?.page?.totalPages || 0;
  const totalElements = userListResponse?.page?.totalElements || 0;
  ...
  const handleDelete = useCallback(
    ...
    [openDeleteModal, deleteUserMutation],
  );
      ...
      <div className="animate-page-fade space-y-6">
        ...
        <div className="animate-slide-up flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-300">
              Sistem kullanıcılarını yönetin ve izleyin ({totalElements}{" "}
              kullanıcı)
            </p>
          </div>
          <Button
        ...
        <Card className="animate-scale-in border-0 bg-white/80 shadow-lg backdrop-blur-sm dark:bg-gray-800/80">
          <CardHeader className="border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50 dark:border-gray-700 dark:from-gray-700 dark:to-gray-800">
            ...
            <CardDescription className="text-gray-600 dark:text-gray-300">
              Sistemdeki {totalElements} kullanıcının listesi
            </CardDescription>
          </CardHeader>
          <CardContent className="p-6">
            {isLoading ? (
                  ...
                  <TableBody>
                    {users.map((user: User, index) => (
                      ...
                      >
                        ...
                        <TableCell className="py-4">
                          <div className="text-gray-600 transition-colors group-hover:text-gray-800 dark:text-gray-300 dark:group-hover:text-gray-100">
                            {user.email}
                          </div>
                        </TableCell>
                        <TableCell className="py-4">
